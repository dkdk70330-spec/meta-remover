(() => {
  ...
  function renderDownloads(){el.readyCount.textContent=`${state.ready.length}개 준비`;if(!state.ready.length){el.downloadList.innerHTML='<p class="empty-state">제거가 완료되면 여기에 변환 이미지가 표시됩니다.</p>';return;}el.downloadList.innerHTML=state.ready.map(item=>`<button class="download-thumb-only ${item.id===state.selectedReadyId?'is-selected':''}" type="button" data-ready-id="${escapeHtml(item.id)}" title="${escapeHtml(outputName(item.name))}"><img src="${escapeHtml(item.url)}" alt="${escapeHtml(outputName(item.name))}"><span class="item-download" data-download-ready="${escapeHtml(item.id)}">다운로드</span></button>`).join("");}
