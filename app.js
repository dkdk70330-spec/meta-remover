(() => {
  ...
  const el = {
    dropZone: $("#dropZone"), fileInput: $("#fileInput"), folderInput: $("#folderInput"), pasteUploadButton: $("#pasteUploadButton"), clearUploadButton: $("#clearUploadButton"),
  ...
  function renderUploads(){if(!state.uploads.length){el.uploadList.innerHTML='<p class="empty-state">아직 업로드된 PNG가 없습니다.</p>';return;}el.uploadList.innerHTML=state.uploads.map(item=>`<div class="thumb-item"><img src="${escapeHtml(item.url)}" alt=""><span class="thumb-copy"><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.relativePath)} · ${formatBytes(item.blob.size)}</span></span><em class="thumb-state ${item.status==='error'?'found':item.status==='done'?'clean':'wait'}">${item.status==='processing'?'처리 중':item.status==='done'?'제거 완료':item.status==='error'?escapeHtml(item.error):'대기'}</em></div>`).join("");}
